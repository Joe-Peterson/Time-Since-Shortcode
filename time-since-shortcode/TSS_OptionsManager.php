...
class TSS_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'time-since-shortcode'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'time-since-shortcode'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'time-since-shortcode'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'time-since-shortcode'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'time-since-shortcode');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'time-since-shortcode'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'time-since-shortcode');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'time-since-shortcode'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'time-since-shortcode') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'time-since-shortcode');
            case 'false':
                return __('false', 'time-since-shortcode');

            case 'Administrator':
                return __('Administrator', 'time-since-shortcode');
            case 'Editor':
                return __('Editor', 'time-since-shortcode');
            case 'Author':
                return __('Author', 'time-since-shortcode');
            case 'Contributor':
                return __('Contributor', 'time-since-shortcode');
            case 'Subscriber':
                return __('Subscriber', 'time-since-shortcode');
            case 'Anyone':
                return __('Anyone', 'time-since-shortcode');
        }
        return $optionValue;
    }
